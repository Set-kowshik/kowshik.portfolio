...
const CaseStudy = () => {
  ...
  return <div className="min-h-screen bg-background">
      <div className="fixed top-6 left-6 z-50">
        <Button onClick={() => navigate('/')} variant="glass" size="sm" className="backdrop-blur-lg border-white/20 bg-slate-500 hover:bg-slate-400">
          <ArrowLeft size={16} className="mr-2" />
          Back to Portfolio
        </Button>
      </div>
      <iframe src={caseStudyPath} title="Case Study" className="w-full h-screen border-0" style={{
      display: 'block'
    }} />
    </div>;
};
